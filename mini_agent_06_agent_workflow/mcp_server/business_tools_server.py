...
ZOO_DATA= {
  "판다": {
    "location": "아시아 동물관",
    "feeding_time": "11:00, 15:30",
    "note": "관람 대기줄이 길 수 있습니다."
  },
  "실내관": {
    "location": "열대우림관",
    "animals": ["나무늘보", "앵무새", "카피바라"],
    "note": "우천 시 추천"
  }
}

@mcp.tool()
def zoo_lookup(query: str) -> dict:
    """동물원 내 동물, 전시관, 프로그램 정보를 검색합니다.

    Args:
        query: 찾을 동물 또는 시설 이름. 예: 판다, 실내관
    """
    keyword = query.strip()

    if keyword in ZOO_DATA:
        return ZOO_DATA[keyword]

    return {
        "found": False,
        "message": f"'{keyword}'에 대한 정보를 찾지 못했습니다."
    }
